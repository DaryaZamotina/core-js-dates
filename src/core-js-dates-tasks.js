...
function getTime(date) {
  ...
  const secNew = addZero(sec);
  const minNew = addZero(min);
  const hrNew = addZero(hr);
  const res = `${hrNew}:${minNew}:${secNew}`;
  return res;
}
...
function getDayName(date) {
  ...
  const newDate = new Date(date);
  const number = newDate.getDay();
  const res = week[number];
  return res;
}
...
function getNextFriday(date) {
  ...
  if (newFriday - data.getDay() >= 0)
    resultDay.setDate(data.getDate() + (newFriday - data.getDay()));
  else resultDay.setDate(data.getDate() + 7 + (newFriday - data.getDay()));
  return resultDay;
}
